...
impl VirtServerManager {
    ...
    pub fn checkpoint_virt_server(&self, rpc_id: u64, ckp_path: &str) -> Result<(),String> {
        ...
        let recv_bytes = self.message_queue.recv_type_timed(virt_server.recv_id, Duration::from_secs(3600)).map_err(|e| format!("Error receiving message from virt server: {}", e))?;
