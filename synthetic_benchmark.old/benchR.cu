#include <stdio.h>
#include <stdlib.h>
#include <cuda.h>
#include <time.h>
#include <sys/mman.h>
#include <sys/stat.h>        /* For mode constants */
#include <fcntl.h>
#include <pthread.h>

#define NUM_LAUNCHES 100


#define HANDLE_ERROR(call) \
do { \
    cudaError_t err = call; \
    if (err != cudaSuccess) { \
        printf("CUDA error: %s\n", cudaGetErrorString(err)); \
        exit(1); \
    } \
} while(0)


__global__ void add(long iterations, long *d_a, long *d_b, long *d_c) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    long i = blockIdx.x * blockDim.x + threadIdx.x;
    int b = blockIdx.x + threadIdx.x + 32;


    for (long k = 0; k < iterations; k++) {

        for (long j = 0; j < iterations; j++) {
            i += b;
            i -= b;
            i *= b;
        }
    }
    
    d_c[idx] += 2 * i + d_b[idx];
    // d_a[idx] += i + d_b[idx];
}

void *get_random_array(long long size) {
    char *arr = (char*)malloc(size);

    if (arr == NULL) {
        fprintf(stderr, "Unable to allocate array on cpu\n");
        return NULL;
    }

    for (long long i = 0; i < size; i++) {
        arr[i] = (char)(rand() % CHAR_MAX);
    }
    return arr;
}



int main(int argc, char* argv[]) {

    srand(0x112);

    if (argc != 7) {
        fprintf(stderr, "Usage: %s <mem-size(KB)> <num-threads> <num-iterations> <sync-call-iteration 0-100 (100 for no sync)> <result-filename> /shm-name\n", argv[0]);
        return -1;
    }

    long long mem_size = strtoll(argv[1], NULL, 10);
    long long num_threads = strtoll(argv[2], NULL, 10);
    long long num_iterations = strtoll(argv[3], NULL, 10);
    long long sync_rate = strtoll(argv[4], NULL, 10);
    const char* result_f_name = argv[5];
    const char* shm_name = argv[6];
    cudaStream_t stream;
    cudaStreamCreate(&stream);

    int shmfd = shm_open(shm_name, O_RDWR, 0666);
    if (shmfd == -1) {
        fprintf(stderr, "error opening shared memory\n");
        return 1;
    }

    pthread_barrier_t *barrier = (pthread_barrier_t *)mmap(0, sizeof(pthread_barrier_t), PROT_READ | PROT_WRITE, MAP_SHARED, shmfd, 0);
    
    if (barrier == (void *)-1) {
        fprintf(stderr, "error mapping shared memory\n");
        return 1;
    }



    long ktimes[NUM_LAUNCHES];


    long long mem_bytes = mem_size << 10;
    
    if ((num_threads * 8) > mem_bytes) {
        fprintf(stderr, "Too many threads\n");
        return 1;
    }

    long *h_a = (long *)get_random_array(mem_bytes);

    long *h_x = (long *)calloc(mem_bytes, 1);
    long *h_y = (long *)calloc(mem_bytes, 1);
    long *h_z = (long *)calloc(mem_bytes, 1);

    long *d_a;
    long *d_b;
    long *d_c;

    HANDLE_ERROR(cudaStreamSynchronize(stream));
    pthread_barrier_wait(barrier);
    
    timespec ts_start, ts_end, ts_kstart, ts_kend;
    clock_gettime(CLOCK_MONOTONIC, &ts_start);

    HANDLE_ERROR(cudaMalloc(&d_a, mem_bytes));
    HANDLE_ERROR(cudaMalloc(&d_b, mem_bytes));
    HANDLE_ERROR(cudaMalloc(&d_c, mem_bytes));

    HANDLE_ERROR(cudaMemset(d_c, 0, mem_bytes));
    HANDLE_ERROR(cudaMemcpy(d_a, h_a, mem_bytes, cudaMemcpyHostToDevice));
    HANDLE_ERROR(cudaMemcpy(d_b, h_a, mem_bytes, cudaMemcpyHostToDevice));

    long long grid_size = num_threads >> 5;
    long long block_size = 32;
    long long sync_call_count = 0;

    FILE* fp = fopen(result_f_name, "w");
    if (fp == NULL) {
        fprintf(stderr, "Unable to open file for writing: %s\n", result_f_name);
        return 1;
    }

    for (int i = 0; i < NUM_LAUNCHES; i++) {
        clock_gettime(CLOCK_MONOTONIC, &ts_kstart);
        
        add<<<grid_size,block_size, 0, stream>>>(num_iterations, d_a, d_b, d_c);
	if(sync_rate == 0) {
        	HANDLE_ERROR(cudaStreamSynchronize(stream));
		sync_call_count = 0;
	}
	else {
		sync_call_count++;
	}

        clock_gettime(CLOCK_MONOTONIC, &ts_kend);

        ktimes[i] = (ts_kend.tv_sec - ts_kstart.tv_sec) * 1000 + (ts_kend.tv_nsec - ts_kstart.tv_nsec) / 1000000;
        fprintf(fp, "kernel-time: loop = %d %lu\n", i, ktimes[i]);

    }
    if(sync_rate != 0) {
        HANDLE_ERROR(cudaStreamSynchronize(stream));
    }

    HANDLE_ERROR(cudaMemcpy(h_x, d_a, mem_bytes, cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(h_y, d_b, mem_bytes, cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(h_z, d_c, mem_bytes, cudaMemcpyDeviceToHost));
    
    clock_gettime(CLOCK_MONOTONIC, &ts_end);

    long res = 0;
    for (long long i = 0; i < mem_bytes; i++) {
	    res +=((char *)h_x)[i] + ((char *)h_y)[i] + ((char *)h_z)[i];
    }
    int time_ms = (ts_end.tv_sec - ts_start.tv_sec) * 1000 + (ts_end.tv_nsec - ts_start.tv_nsec) / 1000000;
    //printf("Time: %d ms\n", time_ms);
    //printf("Result: %ld\n\n", res);

    cudaFree(d_a);
    cudaFree(d_b);
    cudaFree(d_c);

    free(h_a);

    free(h_x);
    free(h_y);
    free(h_z);



//    for (int i = 0; i < NUM_LAUNCHES; i++) {
 //       fprintf(fp, "kernel-time: %ld\n", ktimes[i]);
 //   }
    fprintf(fp, "time: %ld\n", time_ms);
    fprintf(fp, "Result: %ld\n", res);

    fclose(fp);

    shm_unlink(shm_name);

    return 0;
}
